...
Public Class ClsCtlBooking : Implements InfProses
    Public Function InsertData(Ob As Object) As OdbcCommand Implements InfProses.InsertData
        ...
        data = Ob
        CMD = New OdbcCommand("insert into tbl_transaksi(tgl_sewa, tgl_kembali, id_user, id_driver, kd_jenismobil, hari, harga, total_harga) values('" & data.TglSewa & "','" & data.TglKembali & "','" & data.IdUser & "'," & data.IdDriver & ",'" & data.KdJenismobil & "','" & data.Hari & "','" & data.Harga & "','" & data.TotalHarga & "')", Koneksiopen)
